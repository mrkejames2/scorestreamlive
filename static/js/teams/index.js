...
let teams=[], editingTeam=null, generation=0;
...
async function loadTeams(){
  const g=++generation;setStatus("loading","LOADING");refreshButton.disabled=true;notice(errorEl,"");
  ...
  finally{if(g===generation)refreshButton.disabled=false;}
}
...
function closeModal(){modal.classList.add("hidden");document.body.classList.remove("modal-open");editingTeam=null;}
async function saveTeam(e){
  e.preventDefault();notice(formError,"");const save=$("#save-team");save.disabled=true;setStatus("loading","SAVING");
  try{
...
$("#create-team").onclick=()=>openModal();document.querySelector(".empty-create-team").onclick=()=>openModal();
