...
def _get_db_port():
    try: return int(os.getenv("DB_PORT", "5432"))
    except (ValueError, TypeError): return 5432

def _get_team_logo_max_bytes():
    try: value=int(os.getenv("TEAM_LOGO_MAX_BYTES","2097152"))
    except (ValueError,TypeError): return 2*1024*1024
    return value if value>0 else 2*1024*1024

def _get_club_branding_max_bytes():
    try: value=int(os.getenv("CLUB_BRANDING_MAX_BYTES","2097152"))
    except (ValueError,TypeError): return 2*1024*1024
    return value if value>0 else 2*1024*1024
