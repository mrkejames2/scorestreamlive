...
c=$(curl -sS -o "$T" -w "%{http_code}" -X POST "$BASE_URL/api/teams" -H 'Content-Type: application/json' --data "{\"name\":\"M13-E Validation $(date +%s)\"}"||true)
...
for row in '9|Zoe|Zulu' '2|Adam|Alpha' '|No|Number';do
 ...
 if [[ -n "$J" ]];then JVAL="$J";else JVAL="null";fi
...
echo "[3/6] Checking roster management UX surface..."
curl -fsS "$BASE_URL/teams/$ID">"$T"||true
for m in 'M13-E' 'id="roster-search"' 'id="roster-sort"' 'id="roster-visible-count"' 'id="jersey-warning"' 'No matching players';do
 grep -Fq "$m" "$T"&&ok||bad "Missing M13-E UI marker $m"
done
...
echo "========================================";echo "ScoreStreamLive M13-E Validation Summary";echo "BASE_URL: $BASE_URL";echo "MODE: $VALIDATION_MODE";echo "========================================"
[[ $F == 0 ]]&&echo "M13-E ............... PASS   $P passed / 0 failed"||echo "M13-E ............... FAIL   $P passed / $F failed"
[[ $RC == 0 ]]&&echo "M13-D cumulative .... PASS"||echo "M13-D cumulative .... FAIL"
echo "========================================"
