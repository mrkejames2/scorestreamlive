...
if [[ "$VALIDATION_MODE" == "local" ]]; then
  for m in 'id="clock-duration-form"' 'value="1"' 'value="20"' 'value="25"' 'value="30"' 'value="35"' 'value="40"' 'value="45"' 'Current Half Length'; do
  ...
  grep -Fq 'const CLOCK_DURATION_MINUTES = [1, 20, 25, 30, 35, 40, 45];' static/js/control/control.js || { echo "FAIL presets missing"; fail=1; }
  ...
  if grep -Fq 'return Math.min(elapsed, duration);' static/js/control/clock.js; then
    echo "FAIL Control still freezes at regulation threshold"
    fail=1
  else
    echo "PASS Control clock continues past regulation"
  fi
  ...
  if grep -Fq 'return Math.min(elapsed, duration);' static/js/overlay/overlay.js; then
    echo "FAIL Overlay still freezes at regulation threshold"
    fail=1
  else
    echo "PASS Overlay clock continues past regulation"
  fi
...
for spec in "1:60" "20:1200" "25:1500" "30:1800" "35:2100" "40:2400" "45:2700"; do
  IFS=: read -r m s <<<"$spec"
  [[ $((m*60)) -eq $s ]] && echo "PASS ${m} min = ${s} sec" || { echo "FAIL ${m} min"; fail=1; }
done

for spec in "60:60:0" "61:60:1" "119:60:1" "120:60:2" "1200:1200:0" "1201:1200:1" "1259:1200:1" "1260:1200:2" "2400:2400:0" "2401:2400:1" "2459:2400:1" "2460:2400:2"; do
