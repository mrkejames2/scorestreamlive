...
if [[ "$VALIDATION_MODE" == "local" ]]; then
  check "Team club scope" \
    'club_id: Mapped[Optional[uuid.UUID]]' \
    app/models/team.py

  check "Game club scope" \
    'club_id: Mapped[Optional[uuid.UUID]]' \
    app/models/game.py

  check "TeamManager assignment" \
    'class TeamManager(Base):' \
    app/models/team_manager.py

  check "GameOperator assignment" \
    'class GameOperator(Base):' \
    app/models/game_operator.py
  ...
  check "Manager game-create authorization" \
    'async def can_create_game_with_teams(' \
    app/auth/authorization.py

  check "game create event preserved" \
    'await sio.emit("game:created", _serialize_game(game))' \
    app/services/game_service.py

  check "team create event preserved" \
    'await sio.emit("team:created", _serialize_team(team))' \
    app/services/team_service.py

  check "Game API authenticated" \
    'current_user: User = Depends(require_current_user)' \
    app/api/games.py

  check "Team API authenticated" \
    'current_user: User = Depends(require_current_user)' \
    app/api/teams.py

  check "Game list Club scoped" \
    '_require_club(current_user)' \
    app/api/games.py

  check "Team list Club scoped" \
    '_require_club(current_user)' \
    app/api/teams.py

  check "Game list role scoped" \
    'visible_game_ids(db, current_user)' \
    app/api/games.py

  check "Team list role scoped" \
    'visible_team_ids(' \
    app/api/teams.py

  check "Manager Game creation guarded" \
    'allowed = await can_create_game_with_teams(' \
    app/api/games.py

  check "protected Control Center" \
    'current_user: User = Depends(require_current_user)' \
    app/api/control.py

  check "public Overlay state API" \
    '"/api/public/games/{game_id}/overlay-state"' \
    app/api/control.py

  check "Overlay uses public snapshot" \
    'api(`/api/public/games/${gameId}/overlay-state`)' \
...
if [[ "$control_status" == "401" ]]; then
  echo "PASS unauthenticated Control Center rejected"
else
  echo "FAIL Control Center expected 401 got ${control_status}"
  fail=1
fi
...
if [[ "$overlay_status" == "200" ]]; then
  echo "PASS public Overlay shell preserved"
else
  echo "FAIL public Overlay shell expected 200 got ${overlay_status}"
  fail=1
fi
...
if [[ "$public_state_status" == "404" ]]; then
  echo "PASS public Overlay state bypasses authentication"
else
  echo "FAIL public Overlay state expected 404 for fake Game, got ${public_state_status}"
  fail=1
fi
